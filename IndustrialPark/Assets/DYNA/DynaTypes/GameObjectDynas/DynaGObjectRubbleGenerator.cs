...
namespace IndustrialPark
{
    public class DynaGObjectRubbleGenerator : AssetDYNA, IAssetAddSelected
    {
        ...
        public override string Note => "Version is 5 in Incredibles and 8 in ROTU";
        ...
        [Category(dynaCategoryName)]
        public AssetSingle MinLaunchAngle { get; set; }
        [Category(dynaCategoryName)]
        public AssetSingle MaxLaunchAngle { get; set; }
        [Category(dynaCategoryName)]
        public AssetSingle MinBouncesDistRatio { get; set; }
        [Category(dynaCategoryName)]
        public AssetSingle MaxBouncesDistRatio { get; set; }
        [Category(dynaCategoryName)]
        ...
        public DynaGObjectRubbleGenerator(Section_AHDR AHDR, Game game, Endianness endianness) : base(AHDR, DynaType.game_object__RubbleGenerator, game, endianness)
        {
            using (var reader = new EndianBinaryReader(AHDR.data, endianness))
            {
                ...
                if (Version == 8)
                {
                    MinLaunchAngle = reader.ReadSingle();
                    MaxLaunchAngle = reader.ReadSingle();
                    MinBouncesDistRatio = reader.ReadSingle();
                    MaxBouncesDistRatio = reader.ReadSingle();
                }
                else if (Version == 5)
                {
        ...
        protected override void SerializeDyna(EndianBinaryWriter writer)
        {
            ...
            writer.Write((Version == 5) ? 0x3C : 0x44);
            writer.Write(SpawnRate);
            writer.Write(SpawnLimit);
            if (Version == 8)
            {
                writer.Write(MinLaunchAngle);
                writer.Write(MaxLaunchAngle);
                writer.Write(MinBouncesDistRatio);
                writer.Write(MaxBouncesDistRatio);
            }
            else if (Version == 5)
            {
        ...
        public override void SetDynamicProperties(DynamicTypeDescriptor dt)
        {
            if (Version == 8)
            {
                dt.RemoveProperty("MinVelocity");
                dt.RemoveProperty("MaxVelocity");
            }
            else
            {
                dt.RemoveProperty("MinLaunchAngle");
                dt.RemoveProperty("MaxLaunchAngle");
                dt.RemoveProperty("MinBouncesDistRatio");
                dt.RemoveProperty("MaxBouncesDistRatio");
            }
        }
